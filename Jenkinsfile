    ...
    post {
        ...
        always {
            ...
            mail to: 'qi.jin@supplyframe.cn',
              ...
              body: "${env.JOB_NAME} #${env.BUILD_NUMBER} (${env.BUILD_URL}) on branch ${env.BRANCH_NAME}=${env.PLATFORM} has result: ${currentBuild.currentResult}. change title: ${CHANGE_TITLE}, change author: ${CHANGE_AUTHOR}"
            // withCredentials([string(credentialsId: 'slack-token', variable: 'slackCredentials')]) {
            //     slackSend teamDomain: 'bigeworld',
            //         channel: '#jenkins', 
            //         token: slackCredentials, 
